...
| ID | Title | Depends on | Size | Status | Owner | First-pickup? |
|---|---|---|---|---|---|---|
| C-030 | dictionary-v2 — `ar-v2.md` opt-in: 4 revised keywords (`باسم` for `as`, `مرر` for `pass`, `طالما` for `while`, `يكون` for `is`); backward-compatible via ADR 0010 versioning | C-001 | S | delivered | — | **yes** (Arabic linguists) |
